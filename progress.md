...
- **Doğrulama (bu tur)**: `pnpm test` → **20 dosya / 272 test hepsi yeşil**; `npx tsc --noEmit` → **0 hata**; `pnpm run build` → **EXIT 0, 39/39 sayfa** (yeni rotalar: `/reklam`, `/api/ads`, `/api/admin/data/[resource]`, `/api/admin/overview`); prod `next start` smoke: `/` `/reklam` `/blog` `/projects` `/api/ads` → **hepsi 200**; `/api/ads` payload doğru (slot + HMAC token + stats).
- **Hijyen**: public/github-data.json dışında public/ değişimi yok; sırlı dosya yok (.env.local/schema.sql staged'de yok); `scripts/*` gitignore düzeltmesi (`/scripts/*` + istisnalar — eski `/scripts/` no-op idi) + schema-ads/schema-fixes repoda.
- **Not**: `scripts/schema-ads.sql` DB'ye UYGULANMADI (dashboard session token gerekir — kullanıcı manuel veya yetkili oturum); dev server şu an kapalı, `dev-server.ps1` ile başlatılır. Önceki tur "Sıradaki" maddeleri (alt sayfa ikonları inline, blog 0 yazı) hâlâ açık.
